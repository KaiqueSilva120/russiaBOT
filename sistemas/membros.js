...
const { SlashCommandBuilder, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, DiscordAPIError } = require('discord.js');
...
// Função para criar o embed base
...
// IDs dos cargos em ordem hierárquica
...
// Função que gera as páginas (usada no comando e nos botões)
async function gerarPaginas(client, interactionUser) {
    const guild = client.guilds.cache.first(); // ou pegue via interaction.guild se disponível
    ...
    const pages = [];
    let currentPageEmbed = createBaseEmbed(client, { user: interactionUser });
            ...
            membros.forEach((membro) => {
                ...
                if (totalLength > 1024 || (currentFieldCount >= maxFieldsPerPage && fieldCountForCargo === 0)) {
                    if (currentFieldCount > 0) {
                        ...
                        currentPageEmbed = createBaseEmbed(client, { user: interactionUser });
                        currentFieldCount = 0;
                    }
    ...
    try {
        ...
        if (recentMembersList.length > 0) {
            ...
            if (!lastPage || lastPage.data.fields.length >= maxFieldsPerPage) {
                lastPage = createBaseEmbed(client, { user: interactionUser });
                pages.push(lastPage);
            }
...
module.exports = (client) => {
    ...
    const comandoMembros = {
        ...
        async execute(interaction) {
            ...
            const pages = await gerarPaginas(client, interaction.user);
            let currentPage = 0;

            const getRow = (currentPage) => {
                return new ActionRowBuilder().addComponents(
                    new ButtonBuilder()
                        .setCustomId('membros_prev')
                        .setLabel('Anterior')
                        .setStyle(ButtonStyle.Primary)
                        .setDisabled(currentPage === 0),
                    new ButtonBuilder()
                        .setCustomId('membros_pageinfo')
                        .setLabel(`Página ${currentPage + 1}/${pages.length}`)
                        .setStyle(ButtonStyle.Secondary)
                        .setDisabled(true),
                    new ButtonBuilder()
                        .setCustomId('membros_next')
                        .setLabel('Próximo')
                        .setStyle(ButtonStyle.Primary)
            ...
            await channel.send({ embeds: [pages[currentPage]], components: [getRow(currentPage)] });
    ...
    // Listener global para botões permanentes
    client.on('interactionCreate', async (interaction) => {
        if (!interaction.isButton()) return;
        if (!['membros_prev', 'membros_next'].includes(interaction.customId)) return;

        await interaction.deferUpdate();
        const message = interaction.message;

        let currentPage = 0;
        const match = message.components[0].components[1].data.label.match(/Página (\d+)\/(\d+)/);
        if (match) {
            currentPage = parseInt(match[1], 10) - 1;
        }

        if (interaction.customId === 'membros_prev' && currentPage > 0) {
            currentPage--;
        } else if (interaction.customId === 'membros_next') {
            currentPage++;
        }
        ...
        await message.edit({ embeds: [pages[currentPage]], components: [getRow(currentPage)] });
    });
